...
`ce_damage_rect(center = 目标坐标, angle 0, life 2, dmg 80, w 24, h 24)`（`0x45dfa0`，Remilia 脉冲 / 青眼光束同一个原语）。
...
| `category` / `price_tier` / `weight` | `2`（装备）/ `8`（240）/ `2` | 与零售 `*_OP` 子机卡同价 |
| 子机偏移 | `0x18` | Alice `0x1c` 与 Marisa1 `0x10` 之间 |
| 周期 | 120 帧 | 蓄满没目标就攒着，敌人一进射程立刻劈 |
| 伤害 / 判定 / 寿命 | 80 / 24×24 / 2 帧 | §1.2 |
