...
龙加了与炎魔之王同款的**呼吸浮动**：画面 y = 跟随位置 y + `be_bob_dy`（三角相位过 smoothstep 折成来回，±5 px、96 帧一个来回，不引 libm、头里 `static inline` 守零 x87）。
挡弹判定、光束起点（龙头）、血条都用同一个浮动值，与画面一致。跟随的 lerp 目标不受影响（浮动只叠在显示 / 判定上）。
