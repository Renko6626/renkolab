...
- 模式 8 的顶点缓冲（`FUN_00485110` `0x485110`，`CreateVertexBuffer(0x2d0, FVF 0x102)` = 36 顶点 = 9 块四边形，每顶点 0x14：x,y,z,u,v）：
  基础四边形四角 (−128,−128,0,0,0) (128,−128,0,1,0) (−128,128,0,0,1) (128,128,0,1,1)——**居中**；之后按 y +`0x4b9310` / −`0x4b9354`、
  x 同样平移出 9 块，`DrawPrimitive` 起始 = `(v*3 + h) * 4`，`anchor(0,0)` = 第 0 块 = 居中。`set_sprite` 把 `w×0x4b9088 / h×0x4b9088`
  写进 `vm+0x3d4/+0x3e8` 当矩阵缩放（`0x4b9088` 应为 1/256，未读 🟡）。
