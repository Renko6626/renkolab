# engine/ecl/th16 — TH16 ECL(敌机/弹幕脚本)VM 逆向(新会话入口)
...
> 弹幕引擎**(`engine/bullet/th16/`)。ECL 是东方 gameplay 逻辑的顶层脚本(enemy control language)。
> 你若是**新 Claude 会话**,读完这份 README 应知道目标、规范、环境、和**从哪一刀切**。
>
> ✅ **现状(2026-06-11)**:**ECL VM 核心已基本反完**(✅ 一手 + thecl/ExpHP/Priw8 四源交叉)——执行模型、
> 解释循环、两层 opcode(<300 VM 核心 / ≥300 宿主 syscall)、栈/变量三命名空间、ECL↔弹幕开火接缝全打通。
> **先读 `07-vm-architecture.md`(capstone 宏观图)**,细节见 `00`–`06` 与下方"产出(现状)"。
> **剩余**(都是细节/边角,非主体):`≥300` 各 handler 未逐条反(Priw8 eclmap + ECL-info 已覆盖名+行为)、
> 具体开火 opcode → 弹幕 fire 描述符字段细映射、几个 vestigial 系统码(18/19/20/41)、回填 THTK-Studio 仓库的 `docs/`。
> ⚠️ 下方"起步锚点(starting points)"是**当初的切入点(历史记录)**,不少已被后续 ✅ 解决——**以其行内 ✅/❌ 标注为准**,别再当待办。

## 这是什么 / 三套 VM 的位置

TH16 运行时至少**三套字节码 VM**,别混:
1. **ECL VM**(本文件夹)= 顶层 gameplay 脚本:驱动敌机/Boss/关卡、**发出开火/移动/spawn 指令**。
2. **弹运动 VM**(`engine/bullet/th16/` 已解,`0x413860`)= 每颗弹自带的小段运动脚本(加速/转向/反弹…)。
3. **ANM VM**(`engine/anm/th16/`)= 显示/精灵动画脚本。

ECL 在最上层:**ECL 的"开火"指令 → 创建弹对象 + 灌运动字节码**,这就是 ECL↔弹幕的接缝(见 §A)。

## 目标(1–4 已基本达成,见 §产出 / `07-vm-architecture.md`;保留作范围说明)
...
3. **开火接口**:ECL 开火指令如何把参数(角/速/弹型/散布/anm)映射到弹对象字段(`engine/bullet/th16/01` §5 字段图)。
4. **格式 ↔ 运行时**:把运行时 opcode 与 **`thecl.exe`(thtk)** 反汇编出的 ECL 指令号/语义对应,产出"指令号→行为"表。

## 规范(与父工作区一致)

- **五段证据链**(发现→推测→验证→结论(可信度+版本)→证据(地址/出处)):`METHOD.md`。
...
- 常量/字节可直读 PE:`local/th16.v1.00a/th16.exe`。
- 落盘:命名脚本 `tooling/ghidra/scripts/apply_th16_ecl_names.py`(建议,待建);文档 → 本文件夹。

## 起步锚点(starting points)— 来自 `engine/bullet/th16/01-core-engine.md`(2026-06-09)
...
- ✅ **opcode 解释循环已反编译**:`EclRunContext::ecl_run`(`0x472030`)—— 全部系统 opcode(0–0x5d:RET/CALL/CALL_ASYNC/GOTO/IF/算术/比较/栈/数学)运行时语义 + time 门控 + rank 过滤 + 段式派发(游戏码走 vtable[0]=`ecl_run_over_300`)。**详见 `04-ecl-vm-interpreter.md`**(社区未公开过这块;且 Priw8 eclmap 第 4 源印证 opcode 名)。
  - ⚠️ 游戏 opcode 名+签名已被 **Priw8 `vendor/th16.eclmap`(298 条)** 全覆盖,行为多在 `ECL-info.md` → 全反 `ecl_run_over_300` 边际价值有限。**下一刀选择性挖**:ECL 开火 opcode(Et_* 600 区 / `ecl_enm_create`)→ 弹幕 fire 描述符字段映射(接 `engine/bullet/th16/01` §6,exe 才有的接缝)。

### C. 敌机侧(ECL 的宿主,🟡 继承旧文档)
- 敌人管理器 `DAT_004a6dc0`(+0x180 链表头 / +0x18c 计数 / +0x15c 游标)。
- 敌人对象 ~`0x574c`(独立 `operator_new`),`FUN_0041aa70` spawn 进敌人链;每帧更新疑 `FUN_0041c330`(engine/sht/th16/06 §4)。
...
- ★ 接缝来源:`engine/bullet/th16/01-core-engine.md` §6(spawn `FUN_00412cb0` / 字节码源)、§7 开放问题 1。
- 弹运动 VM(ECL 下游):`engine/bullet/th16/01-core-engine.md` §3(opcode 表)。
- ANM 显示 VM(另一独立 VM):`engine/anm/th16/README.md`。
- PRNG/数学(ECL random opcode 用):`engine/_shared/math-and-prng.md` §3。
- 旧切口(二手,待复核):`engine/sht/th16/06-th16-engine-incisions.md`(§4 敌人);自机弹 ECL 锚点 `engine/sht/th16/03/04`。
