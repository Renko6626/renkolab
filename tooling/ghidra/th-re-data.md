...
## 怎么用(新会话)
1. **想要可读性最大化** → 先做"📥 可导入"那 614 个:写个脚本读 ExpHP `local/vendor/th-re-data/data/th16.v1.00a/funcs.json`,
   对工程里仍 `FUN_` 的地址批量 `rename`(参考 `tooling/ghidra/scripts/apply_th16_ecl_names.py` 的做法;函数名经 MCP rename+save 可落盘)。
   ⚠️ 机械活**写脚本别派 agent**(memory `re-workflow-fanout-cost`)。
2. **想做新研究** → 从"🔬 真·待挖"挑一个**大 + 高 xrefs + 子系统线索你关心**的(或挑一整片子系统),
   按 `engine/ecl/th16/` 那套流程反(一手反编译 + 四源对照 + 落 Ghidra + 写 findings)。
3. 当前推荐主攻:**ANM VM**(AnmVm/AnmManager/AnmLoaded 三类合计待导入 95 + 待挖 45,且是缺的第三套字节码 VM,见 `engine/anm/th16/`)。
...
- **`import_th_re_data_structs.py`** ← ★ 导 ExpHP **类型定义**(structs/unions/enums/typedefs)。导名脚本**不导这些**。
  ...
  - 用法 A(纯 python 生成 C,喂 MCP `parse_type_declaration` 或 Ghidra CParser):`python tooling/ghidra/import_th_re_data_structs.py <DATA_DIR> --emit-c`;`--check` 自检 size(实测 **288/288 通过**)。
  ...
  - **现状(2026-06-12):✅ 全部 158 个 z* 结构体已用法 B 导入并落盘**(types=158 fail=0,zEnemyData/zEnemy/zBullet/zItem/zGui/zLaser*/zStage/zSupervisor/zMainMenu/zEclVm… 全在,category `/`)。player/bomb/SHT 的应用(`PLAYER_PTR→zPlayer*`、`*_BOMB_PTR→zBomb*` + 函数签名)经 REPLACE 原地更新仍生效。见 `engine/player/th16/05` §0.5。
...
## 关联
- ExpHP 名库:`local/vendor/th-re-data`(命名层,无语义)。
- 落盘/driver:memory `ghidra-mcp-save-broken`、`ghidra-re-mcp-tips`;纪律 `METHOD.md`。
- 已完成子系统:ECL `engine/ecl/th16/` · 弹幕 `engine/bullet/th16/` · 主循环 `engine/_shared/frame-loop.md` · 数学/PRNG `engine/_shared/math-and-prng.md`。
