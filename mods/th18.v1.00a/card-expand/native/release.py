...
只覆盖**生成物**：三个 patch 的 th18.v1.00a.js、它们的 files.js、DLL。
...
def main():
    ...
    for p in PATCH_DIRS:
        d = os.path.join(R, p)
        out = {n: zlib.crc32(open(os.path.join(d, n), "rb").read()) & 0xffffffff
               for n in sorted(os.listdir(d)) if n.endswith(".js") and n != "files.js"}
