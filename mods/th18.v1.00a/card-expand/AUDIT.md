...
**O27（2026-09-04）开发辅助，只进 `_test`**：① `cards_dev.js` 的 `retail_weight` / `new_weight`——纯数据，改 cave 表行 `+0x14`，在 `ce_shop_capacity_check` 之前应用（否则池子超 560 的 FAIL 会漏）；跳过 id 0 / 56 / 57 与 weight ∈ {0,6} 的行；`ce_weight_override` 有单测。② `st01.ecl` 源文本入库（`assets/ecl/st01.ecl.txt`，真空壳：头部 + `main` + `LogoEnemy` + `MainBoss`，`main` 里道中段与 `debug22` 全删，零售敌机子程序不留），thecl 直接编；`st01bs.ecl`（满是符卡脚本）由 `make_dev_ecl.py` 现场反编译零售文件、血量常数等比 ÷100（`lifeSet`、`setInterrupt(槽, 血量, 超时, 子)` 的血量、`lifeMarker` 的血量；超时不动）、编回（往返字节一致）。第一版只把 `lifeSet` 改 1、阈值 2100 没动，实跑血条 / 阶段切换乱掉——阈值必须一起缩。③ `start_money`：`BP_ce_test_deck` 第 0 槽（`reset_cards` 开局重建）时写 `MONEY`；实跑：`reset_cards` 一局被调多次（开局三次、捡卡后又一次 `20 -> 400`），第一版每次都写 → 改成只在 `MONEY == 0` 时写（开局那次；玩家恰好 0 金时会再触发，dev 可接受）。④ `start_deck_force` / `start_owned`：卡组格子数来自存档张数 `SCOREFILE+0x5f678+槽*4`，断点只在循环内、加不了格；`start_owned` 在第 0 槽时直接写 `owned[]`（`mgr+0xd70`，`reset_cards` 已先清过，02-lifecycle §6），只标不发卡，给皇家同花顺判定用。不碰引擎代码。**未实跑。**
