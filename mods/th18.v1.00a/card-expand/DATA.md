...
`_test` 还带 `st01.ecl`（源 `assets/ecl/st01.ecl.txt`，空壳）/ `st01bs.ecl`（`make_dev_ecl.py` 现场把零售 boss 的血量常数等比 ÷100：`lifeSet` / `setInterrupt` 阈值 / `lifeMarker`；boss 死时多掉 300 个金钱道具当开发用起始资金）：一关近乎空壳，logo → 对话 → 一发一阶段的 boss → 关底商店。
