...
| `start_money` | 开局金钱（0–9999），初始卡组断点第 0 槽且 `MONEY == 0` 时写入（`reset_cards` 一局多次，只在开局那次生效）|
| `start_deck_force` | true = 第 k 格哪怕不空也换成 `start_deck[k]`；格子数由存档决定（`SCOREFILE+0x5f678`），断点不加格 |
| `start_owned` | 开局把这些 id 直接标进 `owned[]`（不发卡）——测皇家同花顺不受卡组容量限制 |
| `trace` | 桩被调时记日志 |
