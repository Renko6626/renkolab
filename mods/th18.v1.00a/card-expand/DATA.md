...
> 行为（获得时 / 每帧 / 激活时做什么）不在这里：那是 DLL 里的 C 代码，按 id 绑定，见 [`NEXT.md`](NEXT.md)。
...
| **行为** | `native/` 里一个 `.c` | 你，C | 下一轮（行为 SDK）|

先写登记，卡就已经能在游戏里走完一圈（无行为）；再写行为。两层能分开排错。
...
| --- | --- | --- | --- | --- | --- |
| `name` | 文案行 0 | ✅ | — | UTF-8，≤ 63 字节 | **不能含 `%`**：名字会被当 printf 格式串（`0x4873f0` → `0x404e40`）|
