...
🔧 **皇家同花顺**（`cards/royal.c`，五张共用 `.ctor`）：买到第五张黑桃时触发一次——金钱 +800、残机 +2（上限先 +1 钳 7，照 CardLife）、bomb +2（同法照 CardBomb）。判定：ctor 时其余四张已在 `owned[]` 且自己尚未 owned（新获得；每关开始的重调不算）。演出：`ability.anm` script69 弹出金色「ROYAL FLUSH」横幅（层 20，弹出→停留 70 帧→上浮淡出）+ 音效 0x4d。AUDIT O26。
